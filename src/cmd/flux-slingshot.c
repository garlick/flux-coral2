...
static struct optparse_subcommand subcommands[] = {
    ...
    },
    {
        .name = "list",
        .usage = "[OPTIONS]",
        .doc = "List NICs and their CXI service counts",
        .fn = cmd_list,
        .flags = 0,
        .opts = list_opts,
    },
                                       ...
                                       void *match_info)
{
    ...
    for (int i = 0; i < svc_list->count && (all || count == 0); i++) {
                                         ...
                                         int ncores)
{
    ...
    warn ("cxi%u: alloc cxi_svc=%d uid=%u vnis=%s ncores=%d", dev->info.dev_id, e, uid, s ? s : "", ncores);
    free (s);
}
...
/* Read the job eventlog and find the cray-slingshot reservation
 * placed there by the jobtap plugin.  If a start event or fatal
 * exception is found first, then return NULL.  Other errors cause
 * an immediate exit.
 */
static json_t *reservation_from_eventlog (flux_future_t *f)
{
    bool done = false;
    json_t *res = NULL;
    ...
    while (!done) {
        ...
        if (flux_future_wait_for (f, eventlog_timeout) < 0 || flux_job_event_watch_get (f, &s) < 0)
            fatal ("error reading job eventlog: %s", future_strerror (f, errno));
        if (!(entry = eventlog_entry_decode (s))
            || eventlog_entry_parse (entry, NULL, &name, &context) < 0)
            fatal ("error parsing eventlog entry");
        if (streq (name, "start")) {
            done = true;
            ...
            if (json_unpack (context, "{s:o}", "reservation", &res) < 0)
                fatal ("error parsing cray-slingshot event context");
            done = true;
            json_incref (res);
        }
        json_decref (entry);
...
static json_t *lookup_reservation (flux_t *h, flux_jobid_t id)
{
    ...
    res = reservation_from_eventlog (f);
...
static json_t *parse_reservation_vnis (json_t *res)
{
    ...
    if (json_unpack (res, "{s:o}", "vnis", &vnis) < 0 || json_array_size (vnis) < 1
        || json_array_size (vnis) > CXI_SVC_MAX_VNIS)
        return NULL;
...
static int ncores_from_R (flux_future_t *f)
{
    ...
    json_array_foreach (R_lite, index, entry) {
        ...
        if (json_unpack_ex (entry, &jerror, 0, "{s:s s:{s:s}}", "rank", &r, "children", "core", &c) < 0)
...
static int cmd_prolog (optparse_t *p, int argc, char **argv)
{
    ...
    if (!(f_eventlog = flux_job_event_watch (h, id, "eventlog", 0)))
        fatal ("error sending eventlog watch request");
    if (!(res = reservation_from_eventlog (f_eventlog))) {
        if (optparse_hasopt (p, "dry-run"))
...
#else
    char *s = json_dumps (vnis, JSON_COMPACT);
    warn ("no CXI support, ncores %d, vnis=%s", ncores, s ? s : "");
    free (s);
#endif
    json_decref (res);
done:
...
static int cmd_clean (optparse_t *p, int argc, char **argv)
{
    ...
    if (!(vnipool = lookup_vnipool (h)))
        fatal ("cray-slingshot.vni-pool not configured");
    monotime (&t0);
    do {
        if (busycount > 0)
            sleep (1);
#if HAVE_CXI
        busycount = destroy_cxi_service (p, true, match_vnipool, vnipool);
#endif
    } while (busycount > 0 && timeout > 0 && monotime_since (t0) < timeout * 1E3);
...
static int cmd_list (optparse_t *p, int argc, char **argv)
{
    ...
    for (int i = 0; i < dev_list->count; i++) {
        ...
        printf ("%-4s %-1s %-3d %-3d %-3d\n",
                dev_list->info[i].device_name,
                cxil_rh_running (&dev_list->info[i]) ? "↑" : "↓",
                def,
                sys,
                usr);
    }
