...
#ifndef IDSET_FLAG_ALLOC_RR
#define IDSET_FLAG_ALLOC_RR 0
#endif
#include <flux/core.h>
...
json_t *vnipool_reserve (struct vnipool *vp, flux_jobid_t id, int vnicount, flux_error_t *error)
{
    ...
    if (!(vnis = vnipool_alloc_array (vp, vnicount))) {
        errprintf (error, "failed to reserve %d VNI%s (%zu available)", vnicount, vnicount > 1 ? "s" : "", idset_count (vp->pool));
        errno = ENOSPC;
        return NULL;
    }
    if (!(res = json_pack ("{s:O}", "vnis", vnis))
        || json_object_set_new (vp->jobs, key, res) < 0) {
...
int vnipool_release (struct vnipool *vp, flux_jobid_t id, flux_error_t *error)
{
    ...
    if (!(job = vnipool_lookup (vp, id, error)))
        return -1;
    if (!(vnis = json_object_get (job, "vnis"))) {
        errprintf (error, "error parsing job %s reservation", key);
        errno = EINVAL;
        return -1;
    }
...
/* (Re-)configure the VNI pool using vni-pool value from TOML config.
 * The value of vni-pool may be NULL, which means "no pool configured".
